...
class WebGLGJKContactSolver
 {
  ...
  final Float32List simplex = new Float32List(36);
  int numVertices = 0;
  final Float32List closest = new Float32List(6);
  final Float32List cachedCoords = new Float32List(4);
  final Float32List tempCoords = new Float32List(4);
  ...
  void reduceVertices(Float32List coords) {
  ...
  bool updateClosestPoints()
   {
    ...
    Float32List coords = this.cachedCoords;
  ...
  bool closestPointTetrahedron(Float32List coords) {
    var simplex = this.simplex;
    ...
    bool sideABC = ((signOrigin * signD) <= 0);
    ...
    signOrigin = -((a0 * n0) + (a1 * n1) + (a2 * n2));
    bool sideACD = ((signOrigin * signD) <= 0);
    ...
    signOrigin = -((a0 * n0) + (a1 * n1) + (a2 * n2));
    bool sideADB = ((signOrigin * signD) <= 0);
    ...
    bool sideBDC = ((signOrigin * signD) <= 0);
    ...
    var minSqDist = double.MAX_FINITE;//Number.MAX_VALUE;
    var sqDist;
  ...
  double closestPointTriangle(int a, int b, int c, Float32List coords, [bool computeDistance = false]) {
    var simplex = this.simplex;
    ...
    double dot3 = ((b0 * ba0) + (b1 * ba1) + (b2 * ba2));
    double dot4 = ((b0 * ca0) + (b1 * ca1) + (b2 * ca2));
    if (dot3 >= 0.0 && dot4 <= dot3)  // Origin in region outside of B
    {
      coords[1] = 1.0;
      coords[0] = coords[2] = 0.0;
      if (computeDistance) {
