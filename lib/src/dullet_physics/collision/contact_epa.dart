...
class WebGLContactEPA {
  ...
  static const int MAX_FACES = 128;

  // populate vertex and face pools
  Float32List vertex_store = new Float32List(MAX_VERTICES * 9);

  final Hs hull = new Hs();
  final Hs stock = new Hs();
  ...
  WebGLContactEPA() {
    int i;
    List<ContactFace> face_store = new List<ContactFace>(MAX_FACES);
    for (i = 0; i < MAX_FACES; i += 1) {
      face_store[i] = new ContactFace();
    }

    // populat stock with all faces
    for (i = 0; i < MAX_FACES; i += 1) {
      append(stock, face_store[MAX_FACES - i - 1]);
    }
  }

  void bind(ContactFace faceA, int edgeA, ContactFace faceB, int edgeB) {
  ...
  void append(Hs list, ContactFace face) {
    face.leaf0 = null;
  ...
  void remove(Hs list, ContactFace face) {
    ContactFace leaf0 = face.leaf0;
    ContactFace leaf1 = face.leaf1;
    if (leaf1 != null) {
      leaf1.leaf0 = leaf0;
    }
    if (leaf0 != null) {
  ...
  ContactFace buildNewFace(int a, int b, int c, bool forced) {
  ...
  bool expandFace(int pass, int w, ContactFace face, int edge, Horizon horizon) {
    if (face.pass != pass) {
      var fn = face.normal;
      var fn0 = fn[0];
      var fn1 = fn[1];
      ...
      if ((((fn0 * w0) + (fn1 * w1) + (fn2 * w2)) - face.distance) < -1e-6) {
        var newFace = buildNewFace(face.vertex[edge1], face.vertex[edge], w, false);
        if (newFace != null) {
          bind(newFace, 0, face, edge);
          if (horizon.cf != null) {
            bind(horizon.cf, 1, newFace, 2);
          } else {
            horizon.ff = newFace;
          }
          horizon.cf = newFace;
  ...
  double evaluate(Float32List gjkSimplex, WebGLPhysicsTOIEvent cache, Matrix43 xformA, Matrix43 xformB) {
    ...
    var t0 = buildNewFace(0, 9, 18, true);
    var t1 = buildNewFace(9, 0, 27, true);
    var t2 = buildNewFace(18, 9, 27, true);
    var t3 = buildNewFace(0, 18, 27, true);
    ...
    var scale = 1.0 / (alpha + beta + gamma);
    alpha *= scale;
    beta *= scale;
    gamma *= scale;
...
distance : 0,
vertex : new Uint16List(3),
adjFace : [null, null, null],
edge : new Uint16List(3),
