...
class WebGLPhysicsPlaneShape implements WebGLPhysicsShape {
    ...
    WebGLPhysicsPlaneShape(var param) {
      //var retp = new WebGLPhysicsShape();
      //var p = this;
      //retp._private = p;
      //p._public = retp;

      collisionRadius = (param.margin != null) ? param.margin : 0.04;
      distance = param.distance;
      normal = param.normal.clone();
      ...
      if (normal[0].abs() == 1) {
        halfExtents = new Vector3(distance.abs(), maxValue, maxValue);
      } else if (normal[1].abs() == 1) {
        halfExtents = new Vector3(maxValue, distance.abs(), maxValue);
      } else if (normal[2].abs() == 1) {
        halfExtents = new Vector3(maxValue, maxValue, distance.abs());
      }

      center = null;
      inertia = new Vector3.zero();
