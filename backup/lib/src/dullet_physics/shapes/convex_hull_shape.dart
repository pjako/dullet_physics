...
//
class WebGLPhysicsConvexHullShape implements WebGLPhysicsShape
{
    ...
    WebGLPhysicsConvexHullShape(params) {
        ...
        c.points = new Float32List(points);
        c.radius = Math.sqrt((h0 * h0) + (h1 * h1) + (h2 * h2));
        c.halfExtents = new Vector3(h0, h1, h2);
        if (c0 != 0 || c1 != 0 || c2 != 0)
        {
            c.center = new Vector3(c0, c1, c2);
        }
        else
        {
            c.center = null;
        }
        c.inertia = new Vector3(massRatio * (ly + lz),
                                  massRatio * (lx + lz),
                                  massRatio * (lx + ly));
